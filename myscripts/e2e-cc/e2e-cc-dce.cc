...
main(int argc, char* argv[])
{
    ...
    LogComponentEnable("E2EDceApplication", LOG_LEVEL_WARN);
    ...
    auto& dceApplicationConfigs = configParser.GetDceApplicationArgs();
    for (auto& config : dceApplicationConfigs)
    {
        Ptr<E2EDceApplication> application = E2EDceApplication::CreateDceApplication(config);
        auto host = root->GetE2EComponentParent<E2EHost>(application->GetIdPath());
        NS_ABORT_MSG_UNLESS(host,
            "Host for DCE application '" << application->GetId() << "' not found");
        application->AddToHost(*host);
    }
